...
import org.exthmui.settings.deviceinfo.mydevice.ExthmMyDeviceFragment;
...
public class DeviceNameWarningDialog extends InstrumentedDialogFragment
    ...
    public static void show(ExthmMyDeviceFragment host) {
        final FragmentManager manager = host.getActivity().getSupportFragmentManager();
        if (manager.findFragmentByTag(TAG) != null) {
            return;
        }
    ...
    public void onClick(DialogInterface dialog, int which) {
        final ExthmMyDeviceFragment host = (ExthmMyDeviceFragment) getTargetFragment();
        host.onSetDeviceNameConfirm(which == DialogInterface.BUTTON_POSITIVE);
    }
}
